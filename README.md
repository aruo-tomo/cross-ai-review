...
Use this skill when you want a second AI to challenge a draft before you share it. It needs no API key, token, script, or special integration for its default manual workflow. One AI creates the work; a fresh second chat reviews a complete packet; the first AI applies clear fixes and asks you to make the remaining decisions.
...
3. Choose the handoff method:
   ...
   - **CLI relay (advanced):** with active access to both services, let the executor use the other model's installed, authenticated CLI as the reviewer.
...
For a direct model-to-model handoff, choose one of these pairings:
